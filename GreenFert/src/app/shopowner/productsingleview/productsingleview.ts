...
@Component({
  selector: 'app-productsingleview',
  imports: [CommonModule, RouterLink],
  templateUrl: './productsingleview.html',
  styleUrl: './productsingleview.scss',
})
export class Productsingleview {
 product_id:any;
product:any[]=[];
public iarray: any[] = [];
  quantity: any = {};
   ...
   buy(did: string) {
    this.dbservice.Buy({ loginid: localStorage.getItem('login_id'), did: did,totalprice:this.totalprice,quantity:this.quantity }).then((confirmation: any) => {
      if (confirmation.message == "success") {
        ...
        this.rout.navigate(['/smaster/spaddress']);
